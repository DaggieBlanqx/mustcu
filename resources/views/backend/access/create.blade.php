...
@section('content')
    {{ Form::open(['route' => 'admin.access.user.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post']) }}
        ...
        <div class="box box-success" id="access-management">
            ...
            <div class="box-body">
                ...
                <div class="form-group">
                    {{ Form::label('user_type', 'User Type', ['class' => 'col-lg-2 control-label']) }}
                    <div class="col-lg-10">
                        <select v-model="user_type" name="user_type" class="form-control">
                            <option value="{{config('access.user_types.station_manager')}}">Station Manager</option>
                            <option value="{{config('access.user_types.station_employee')}}">Station Employee</option>
                            <option value="{{config('access.user_types.company_manager')}}">Company Manager</option>
                            <option value="{{config('access.user_types.company_employee')}}">Company Employee</option>
                        </select>
                    </div><!--col-lg-10-->
                </div><!--form control-->
...
@section('after-scripts')
    <script>
        var accessmgt = new Vue({
            el: '#access-management',
            data: {
                user_type: {{(old('user_type') != '' || old('user_type') != null) ? old('user_type') : 0}},
                station: 0,
	            employee_station:-1
            }
        })
